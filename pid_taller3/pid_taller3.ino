...
double u_de_control = 0;
double integrativo = 0;
double pwm_out = 0;
...
void ciclo_control()
{
  ...
  if (delta_t >= CICLO_CONTROL_TIEMPO)
  {
    ...
    /************* AGREGUE EL CODIGO AQUI **************/
     ...
     // escribir en esta variable el pwm que se manda al motor (como valor positivo/negativo) en 55 se  mueve
...
//definimos el error:
  ...
  u_de_control = PID_P*error + PID_I*integrativo + PID_D*derivativo;
  if(u_de_control < 0){
    pwm = u_de_control - PWM_MIN;
  }
  if(u_de_control > 0){
    pwm = u_de_control + PWM_MIN;
  }
  ...
  // pwm_out = u_de_control + sign(u_de_control)*PWM_MIN;
  set_motor_pwm(pwm);
//Serial.print("error: "); Serial.println(error,10);
//Serial.print("I: "); Serial.println(integrativo,10);
//Serial.print("D: "); Serial.println(derivativo,10);
//Serial.print("P: "); Serial.println(error,10);
//Serial.print("PWM_OUT: "); Serial.println(pwm,10);
